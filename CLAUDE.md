...
- **GitHub Pages base path**: The site is currently at `porchlight.github.io/heartwoodgc/` so `astro.config.mjs` has `site: 'https://porchlight.github.io'` and `base: '/heartwoodgc/'`. The trailing slash on `base` is **required** — without it `import.meta.env.BASE_URL` becomes `/heartwoodgc` and asset paths break (e.g. `/heartwoodgcicon.svg`). When the custom domain (`heartwoodgc.com`) is set up, the **only change needed** is in `astro.config.mjs`: change `site` to `'https://heartwoodgc.com'` and remove the `base` line. All component paths use `import.meta.env.BASE_URL` which Astro derives automatically from the `base` config (defaults to `/` when no `base` is set), so no other files need to change.
